...
```sh
HACDAO_ALLOW_CLOUD_WRITES=true python migrate_to_cloudflare.py --slug demo --limit 1
```
...
`--batch-upload --bundle-size 50` là chế độ opt-in cần thử trên truyện nhỏ. Đã kiểm thử round-trip bundle bằng fixture local; chưa kiểm chứng dữ liệu production. Không chạy nhiều sync job cùng slug. Hai syncer API dùng chung ngân sách mặc định 0; xem [kiểm soát chi phí](docs/cost-controls.md).
...
`restore_from_cloudflare.py` đọc D1/R2 remote và ghi local. Chạy trên bản sao/thư mục phục hồi riêng, đã cài dependency và Wrangler, sau khi đọc script; không coi đây là lệnh dry-run. Nó hỗ trợ object riêng/bundle, giữ checkpoint truyện lỗi và exit 1 nếu phục hồi thiếu chương. File tồn tại được bỏ qua, không được xác minh checksum tự động. Nó không thay thế backup tài khoản.
...
| Admin cloud trả 404 | Phiên bản Worker đang chạy có route mới chưa; đúng method/path chưa |
...
| Job Actions xanh nhưng không có dữ liệu mới | Kiểm tra số chương thực tế; bản mới không nuốt lỗi sync/commit/push |
