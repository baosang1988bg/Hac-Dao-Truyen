...
async def process_batch_async(translator, profile, batch_items, previous_summary=""):
    """
    batch_items: list of (raw_path, out_path)
    """
    batch_data = []
    ...
    # Translate using thread pool since translate_batch is synchronous
...
async def main():
    ...
    async def run_batch_with_sem(batch):
        ...
        async with semaphore:
            try:
                # We carry over the summary if processed sequentially, but with parallel batches
                # it's harder. We can pass the latest previous_summary or empty. Let's pass previous_summary.
    ...
    # Process all batches in parallel with limit
    tasks = [run_batch_with_sem(b) for b in batches]
    await asyncio.gather(*tasks)

    # ── Post-processing: Merge split chapters ──
    ...
    for raw_name in raw_files:
        ...
        if is_split_original(profile.raw_dir, stem):
            # Check if all parts are translated
